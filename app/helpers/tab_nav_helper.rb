module TabNavHelper
  ...
  def tab_nav_text_classes(selected: false, disabled: false)
    class_names(
      "text-gray-600 group-hover:text-gray-700": !disabled,
  ...
  def tab_nav_icon_classes(disabled: false)
    disabled ? "text-gray-300" : "text-gray-400 group-hover:text-gray-500"
  end
  ...
  def tab_nav_tab_classes(selected: false, disabled: false, dropdown: false)
    class_names(
      ...
      "h-full": dropdown,
      "bg-gray-300 hover:bg-gray-400": !disabled,
      "bg-gray-100 cursor-not-allowed": disabled,
      "border-b-red-400": selected,
      "border-b-gray-400": !selected
    )
  end
  ...
  def tab_nav_dropdown_menu_classes
    class_names(
      ...
      "z-40 absolute mt-1 w-60 rounded-md border-2 border-gray-300 shadow-lg py-1 bg-gray-200 divide-y divide-gray-200"
    )
  end
  ...
  def tab_nav_dropdown_item_classes
    "block w-full text-left py-1.5 px-3 text-gray-500 hover:text-gray-600 hover:bg-gray-300"
  end

  def tab_nav_counter_classes(counter, threshold)
    class_names(
      "text-xs leading-none p-1 rounded-md",
      tab_nav_threshold_classes(counter, threshold)
    )
  end

  def tab_nav_threshold_classes(counter, threshold)
    tab_nav_exceeds_threshold?(counter, threshold) ? "bg-red-50 text-red-600" : "bg-gray-50 text-gray-600"
  end
